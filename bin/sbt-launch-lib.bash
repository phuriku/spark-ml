...
declare -r sbtrepo=http://repo.typesafe.com/typesafe/ivy-releases/org.scala-sbt/sbt-launch
declare -r sbtjar_filename=sbt-launch.jar
declare sbt_version=0.13.11
declare sbtsha128=7435a957b62743d81ae257c60bce11a641aef2a8
...
jar_file () {
  echo "$(dirname "$(realpath "$0")")/${sbtjar_filename}"
}
...
acquire_sbt_jar () {
  ...
  if [[ ! -f "$sbt_jar" ]]; then
      vlog "downloading $sbtjar_filename to $sbt_jar" 1>&2
      if ! curl -o ${sbt_jar} --location --silent --fail --remote-name ${sbtrepo}/${sbt_version}/${sbtjar_filename}; then
        echoerr "Could not find or download launcher jar: $sbt_jar"
        exit 2
      fi
      checksum=`openssl dgst -sha1 ${sbt_jar} | awk '{ print $2 }'`
      if [ "$checksum" != ${sbtsha128} ]; then
        echoerr "bad sbt launcher.  delete $sbt_jar and run this script again."
        exit 3
      fi
  fi
}
